...
class Logger:
    ...
    def shouldPrintMessage(self, timestamp: str, message: str) -> bool:
        ...
        timestamp_epoch = int(datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S %z").timestamp() * 1000)  # O(1) time

        # Check if the message exists in the lastTime dictionary
        if message in self.lastTime:  # O(1) time
            # Check if the time difference between current and last timestamp is less than the limit (5 seconds)
            if timestamp_epoch - self.lastTime[message] < 5000:  # O(1) time
        ...
        self.lastTime[message] = timestamp_epoch  # O(1) time
    ...
    def print_message(self, timestamp: str, message: str):
        ...
        - message: The message content to be printed.
        """
        if self.shouldPrintMessage(timestamp, message):  # O(1) time
            print(f"{timestamp} {message}")  # O(1) time
